...
---
**UPDATE**: I should've clarified something about this blog post before sharing it with people. This blog post describes a proposal that, if correct, could solve a major unsolved problem in physics. Almost everyone I've discussed it with in person thinks it is wrong, and I think it is probably wrong too. The trouble is that everyone I've discussed it with seems to think some parts of this are obviously right and the other parts are obviously wrong, but no one seems to have the same opinion on *which* parts are obviously right and which parts are obviously wrong. So as you read this, keep in mind that there is probably at least one serious, fundamental error somewhere in this post. (Actually, it [might be this](#update-many-worlds).) If you think you see one, please email me at [firstestprinciple@gmail.com](mailto:firstestprinciple@gmail.com), and be *very specific* about what exactly is wrong with this proposal, because many things that may seem to be wrong with it at first glance turn out to not be problems after deeper analysis.
...
<a id="update-many-worlds"></a>**UPDATE**: I've been contacted by a couple people saying that you can describe this by using the [many-worlds interpretation](https://en.wikipedia.org/wiki/Many-worlds_interpretation) of quantum mechanics, and that I was therefore wrong when I wrote that current physics theories can't predict the probabilities of seeing a 0 or a 1 in this situation. I see how this can described as a 2-state [quantum system](http://scottaaronson.com/democritus/lec9.html), but the part I don't understand is what is the right way to assign amplitudes to each of the states, since to the best of my knowledge this hasn't been experimentally tested like the behavior of the fundamental particles has, for example. I'm still looking into this, but if you can explain it to me then I'd really appreciate if you email me at [firstestprinciple@gmail.com](mailto:firstestprinciple@gmail.com). If you are a physicist for which this is really obvious, I'm sorry for asking you to deal with an idiot like me who doesn't know better, but I promise to write a detailed update to the blog post after I understand this better.
