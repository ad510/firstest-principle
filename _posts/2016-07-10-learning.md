...
<a id="update-quantum"></a>**UPDATE** (Oct 2016): A couple people suggested that this can be described by using the [many-worlds interpretation](https://en.wikipedia.org/wiki/Many-worlds_interpretation) of quantum mechanics. The many-worlds interpretation does explain the meaning of quantum mechanics if you can partition the [quantum state of the universe](https://en.wikipedia.org/wiki/Universal_wavefunction) into the subsystem that is "you" and the subsystem that is "not you," but it intentionally does not describe exactly *where* this partition is. A common justification for this is that in practice, any macroscopic quantum state supposedly describing "you" would tend to quickly become correlated, or "[entangled](https://en.wikipedia.org/wiki/Quantum_entanglement)," [with the quantum state of the environment](https://en.wikipedia.org/wiki/Quantum_decoherence), so it's commonly believed that no matter how you partition "you" from the environment, it would not affect which outcome you observe in any practical experiment. But if you can ever put yourself in a simulation, predicting what you can observe using the standard model would require an *extremely precise* understanding of where the partition between "you" and the environment actually is, and the obvious ways of defining that partition would not predict the correct probabilities of seeing a 0 or a 1 from your perspective inside the simulation described above.
...
A [proposal by Scott Aaronson](https://arxiv.org/abs/1306.0159) ([summarized here](http://www.scottaaronson.com/blog/?p=2903) under "Personal Identity") is that maybe you cannot be uploaded to a classical computer after all, but you can still be uploaded to a quantum computer, since unknown quantum information [cannot be copied](https://en.wikipedia.org/wiki/No-cloning_theorem) (though it [can be moved](https://en.wikipedia.org/wiki/Quantum_teleportation)). If this is in fact what the standard model predicts, then I think it's a huge deal that the standard model makes a testable prediction about human consciousness, and trying to upload yourself to a classical computer would be a fantastic test to experimentally distinguish the standard model from digital physics (which I'll explain below). But while I agree about the problems he describes if consciousness is fundamentally copyable, I'm not sure that saying you can only be uploaded to quantum computers is a sufficient solution, because there'd still be other experiments you could do where predicting the result using the standard model would require knowing where the partition between you and the environment is (which the standard model doesn't help you answer). For example, if someone [cuts the bundle of nerves](https://www.youtube.com/watch?v=wfYbgdo8e-8) connecting the 2 halves of your brain, how would you figure out the probabilities of finding yourself in the left or right hemispheres?

Several of my friends have suggested that perhaps this isn't an experiment in the first place, but rather a metaphysical question without a correct answer. But whether you see a 0 or a 1 (or whether you control the left or right side of your body in the split-brain experiment) is definitely something you can observe, and if you can observe it, you can disprove hypotheses about it using the scientific method. In fact, I would go a step further and suggest that the set of questions about [who you will be in the future](https://waitbutwhy.com/2014/12/what-makes-you-you.html) is *exactly* the set of questions that you can answer with experiments, and I'm considering writing a blog post explaining what I mean by that. So I really think we ought to have a physics theory that answers these questions.
