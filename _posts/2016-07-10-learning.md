...
---
**UPDATE**: I should've clarified something about this blog post before sharing it with people. This blog post describes an experiment that I think would distinguish quantum mechanics from [digital physics](https://en.wikipedia.org/wiki/Digital_physics). Almost everyone I've discussed it with in person agrees that this proposal is obviously wrong, and that parts of it are obviously right and other parts are obviously wrong. The trouble is that no one seems to have the same opinion on *which* parts are obviously right and which parts are obviously wrong. So I figured I'd write it up as a blog post to get wider feedback.
...
<a id="update-quantum"></a>**UPDATE** (Oct 2016): A couple people suggested that this can be described by using the [many-worlds interpretation](https://en.wikipedia.org/wiki/Many-worlds_interpretation) of quantum mechanics. The many-worlds interpretation does explain the meaning of quantum mechanics if you partition the [quantum state of the universe](https://en.wikipedia.org/wiki/Universal_wavefunction) into the subsystem of the observer (i.e. "you") and the subsystem of the environment, but it intentionally does not describe *where* this partition is. A common justification for this is that in practice, any macroscopic quantum state supposedly describing "you" would tend to quickly become correlated, or "[entangled](https://en.wikipedia.org/wiki/Quantum_entanglement)," [with the quantum state of the environment](https://en.wikipedia.org/wiki/Quantum_decoherence), so it's commonly believed that no matter how you partition "you" from the environment, it would not affect which outcome you observe in any practical experiment. But if you can ever put yourself in a simulation, predicting what you'd observe using the standard model would require an *extremely precise* understanding of where the partition between "you" and the environment actually is, and the obvious ways of defining that partition would not predict the correct probabilities of seeing a 0 or a 1 from your perspective inside the simulation described above.

For example, if we define the initial quantum state of "you" as the quantum state of the particles in your brain, then thanks to [decoherence](https://en.wikipedia.org/wiki/Quantum_decoherence), the standard model would predict that you would quickly become very entangled with both computers. In other words, quantum mechanics would predict that in a certain sense, you'd be able to observe *both* a 0 and a 1 (like people watching the computers from outside), not one or the other. This would be true for any reasonable-sounding definition of "you" based on the quantum states of physical particles.

If we instead define the quantum state of "you" as the quantum state being simulated in the computers, this would only make sense if the computers are actually simulating quantum mechanics. But even in that case, the [no-cloning theorem](https://en.wikipedia.org/wiki/No-cloning_theorem) and [no-teleportation theorem](https://en.wikipedia.org/wiki/No-teleportation_theorem) imply that any quantum state stored as classical bits cannot be precisely the quantum state of "you," only a quantum state that may or may not be entangled with you.

A [proposal by Scott Aaronson](https://arxiv.org/abs/1306.0159) ([summarized here](http://www.scottaaronson.com/blog/?p=2903) under "Personal Identity") is that maybe you cannot be uploaded to a classical computer after all, but you can still be uploaded to a quantum computer, since unknown quantum information [cannot be copied](https://en.wikipedia.org/wiki/No-cloning_theorem) (though it [can be moved](https://en.wikipedia.org/wiki/Quantum_teleportation)). If this is in fact what the standard model predicts, then I think it's a huge deal that the standard model makes a testable prediction about human consciousness, and trying to upload yourself to a classical computer would be a fantastic test to experimentally distinguish the standard model from digital physics (which I'll explain below). But while I agree about the problems he describes if your subjective identity is fundamentally copyable, I'm not sure that saying your subjective identity is uncopyable solves the underlying problem, because there'd still be other experiments you could do where predicting the result using the standard model would require knowing where the partition between you and the environment is (which the standard model doesn't help you answer). For example, if someone's going to [cut the bundle of nerves](https://www.youtube.com/watch?v=wfYbgdo8e-8) connecting the 2 halves of your brain, how would you come up with probabilities of finding yourself in the left or right hemispheres?

A few of my friends have suggested that perhaps this isn't an experiment in the first place, but rather a metaphysical question without a correct answer. But whether you see a 0 or a 1 (or whether you control the left or right side of your body in the split-brain experiment) is definitely something you can observe, and if you can observe it, you can disprove hypotheses about it using the scientific method. Even though other people won't observe the result that you observe, they can still reproduce the experiment by performing it on themselves. So I think that questions about [who you will be in the future](https://waitbutwhy.com/2014/12/what-makes-you-you.html) can be answered using the scientific method, and we really ought to have a physics theory that tries to answer them.

If you think I missed something, please email me at [firstestprinciple@gmail.com](mailto:firstestprinciple@gmail.com) because I am extremely interested in this topic.
