...
A [proposal by Scott Aaronson](https://arxiv.org/abs/1306.0159) ([summarized here](http://www.scottaaronson.com/blog/?p=2903) under "Personal Identity") is that maybe you cannot be uploaded to a classical computer after all, but you can still be uploaded to a quantum computer, since unknown quantum information [cannot be copied](https://en.wikipedia.org/wiki/No-cloning_theorem) (though it [can be moved](https://en.wikipedia.org/wiki/Quantum_teleportation)). If this is in fact what the standard model predicts, then I think it's a huge deal that the standard model makes a testable prediction about human consciousness, and trying to upload yourself to a classical computer would be a fantastic experiment to distinguish the standard model from digital physics (which I'll explain below). But while I agree about the problems he describes if your subjective identity is fundamentally copyable, I'm not sure that saying your subjective identity is uncopyable fully addresses the underlying problem, which is that the standard model doesn't define where the partition between you and the environment is. For example, if someone's going to [cut the bundle of nerves](https://www.youtube.com/watch?v=wfYbgdo8e-8) connecting the 2 halves of your brain, how would you use the standard model to come up with probabilities of finding yourself in the left or right hemispheres?
