...
**UPDATE**: I've been contacted by a couple people saying that you can describe this by using the [many-worlds interpretation](https://en.wikipedia.org/wiki/Many-worlds_interpretation) of quantum mechanics. The reason I think this is problematic is that to model this with quantum mechanics, you'd want both possibilities to start out with identical quantum states, then have the quantum states diverge after you make a measurement. But in any conventional model of particle physics, the computers in this experiment would never have the same quantum states to begin with. Also, keep in mind that the physics used inside the simulation does not necessarily have to be the same as the physics of real life.
