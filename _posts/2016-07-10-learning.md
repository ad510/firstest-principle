...
Of course, if the brain theory is right and you die before you would see either the 0 or the 1, then this doesn't matter and our existing physics theories would still be valid. But if the data theory is right and there's a non-negligible chance that you *do* see either the 0 or the 1, then that would mean *it's possible to do an experiment in which the most fundamental physics theories we know of today cannot predict the probabilities of you seeing a 0 or a 1!*

In that case, we'd need a new physics theory to fill the gap.

---

**UPDATE**: A couple people suggested that this can be described by using the [many-worlds interpretation](https://en.wikipedia.org/wiki/Many-worlds_interpretation) of quantum mechanics, so let's do that here. This situation definitely has a lot in common with many-worlds, but I don't think it actually applies here because it would require that both computers start out with identical quantum states at the level of subatomic particles. It's extremely unlikely that 2 computers will have exactly the same quantum state just because they store the same bits in memory. But if I missed something and the standard model can indeed predict the amplitudes of seeing a 0 or a 1 from inside the simulation, then I'd really appreciate it if you email an explanation to me at [firstestprinciple@gmail.com](mailto:firstestprinciple@gmail.com).

in QM you can't copy unknown quantum state but you can move it, so it may seem you can answer this by which computer quantum state is moved to
problem with this is to move quantum state like this, you'd have to create entangled pair of qubits for every qubit of every subatomic particle you want to move, and copying the classical bits in memory would not do that
this means SM predict that quantum information will remain in blown up original computer (or human brain for that matter), so if "you" are quantum info, SM predicts you die when your brain is blown up
problem solved? not really b/c we don't know whether information that uniquely determines who "you" are is quantum or classical or what (free will link)
if I misunderstood something, please email me
wikipedia: no cloning theorem, quantum teleportation

For a physicist's take on a very similar question, go to Scott Aaronson's [lecture on free will](http://scottaaronson.com/democritus/lec18.html) and search "Mars Express." In one part, he lists several unsolved questions about its feasibility and says, "These are not metaphysical questions. They are, in principle, empirically answerable ones." You can think of this blog post as proposing one way to empirically answer some of these questions.
