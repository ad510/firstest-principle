...
---
**UPDATE**: I should've clarified something about this blog post before sharing it with people. This blog post describes a proposal to experimentally distinguish the [standard model](https://en.wikipedia.org/wiki/Standard_Model) from [digital physics](https://en.wikipedia.org/wiki/Digital_physics). Almost everyone I've discussed it with in person agrees that it is wrong, and that parts of it are obviously right and other parts are obviously wrong. The trouble is that no one seems to have the same opinion on *which* parts are obviously right and which parts are obviously wrong. If you think you see a mistake in the proposal, please email me at [firstestprinciple@gmail.com](mailto:firstestprinciple@gmail.com), and be *very specific* about what exactly is wrong with it, because many things that may seem to be wrong with it at first glance turn out to not be problems after deeper analysis.
