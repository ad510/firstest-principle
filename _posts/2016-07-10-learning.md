...
<a id="update-quantum"></a>**UPDATE 8/5/2016**: I completely (and belatedly) rewrote this section due to reader feedback. Thanks guys! (Here's the [original version](https://github.com/ad510/firstest-principle/blob/daa34b2e94f8d508b8a4141b58217c8f5445d798/_posts/2016-07-10-learning.md#what-our-physics-theories-cant-predict).)
...
So if you imagine a godlike being who knows what the quantum state of the universe is, here's how they could use the standard model to predict the probabilities of you seeing a 0 or a 1:

First, you define 2 events: one that describes you seeing a 0, and another that describes you seeing a 1. Then, you do some number crunching and calculate the quantum-mechanical [amplitudes](http://scottaaronson.com/democritus/lec9.html) of those events. Finally, you square the amplitudes, and that'll give you the probabilities of seeing a 0 or a 1!

Simple, right? Oh yeah, but before you can do that, you need a good way to define what it means to see a 0 or a 1:

The way measurement works in quantum mechanics is that when you measure something, the quantum state of "you" becomes *entangled*, or correlated with, the quantum state of the thing you measured. For example, if the quantum state of "your thoughts" is entangled with the quantum state of a kitten standing in front of you, the plain English translation is that what you *think* the kitten is doing is probably what the kitten is *actually* doing.

To measure how entangled quantum states are with each other, you can use something called [entropy](https://en.wikipedia.org/wiki/Entropy_of_entanglement). More specifically, you can measure "how much" you see a 0 in the simulation by calculating the entropy of entanglement between the quantum state of "you" and the quantum state of the electrons coming down the wire as a result of the evil prankster pressing the `0` key. If the entropy is extremely close to 0, then it means you do not see a 0. If the entropy is nonzero but kind of low, it means you see a 0 but you're not thinking about it much. If the entropy is large, it means you see a 0 and you're thinking about it a lot. To turn this into an event, you pick a cutoff value, and define the event as whether the entropy will be greater than that cutoff value a few seconds after the evil prankster presses the 0 key.

But before you can calculate the entropy, you have to define 2 things: the quantum state of the electrons coming down the wire, and the quantum state of "you." Defining the quantum state of the electrons coming down the wire is fairly straightforward, but defining the quantum state of "you" is *extremely* hard. What's more, the standard model will *not* give you any guidance on how to do it!

Defining the quantum state of "you" is a famous unsolved problem in physics called the [measurement problem](https://en.wikipedia.org/wiki/Measurement_problem). And this isn't just a matter of [interpretation](https://en.wikipedia.org/wiki/Interpretations_of_quantum_mechanics) or needing enough computational resources. In order to correctly predict the probabilities of seeing a 0 or a 1, you have to take a wild, uninformed guess at how to partition the quantum state of the universe into the subsystem that is "you" and the subsystem that is "not you," and it has to be accurate enough to correctly predict:

1. how "you" move over a brain-computer interface from a brain into a simulation,
2. what happens to "you" when the simulation is copied into 2 other computers, and
3. how strongly "you" entangle with the simulation when it displays either a 0 or a 1.

If your wild guess isn't *that* accurate, then the standard model will make a *very* wrong prediction.

```
entropy isn't a probability, but you can define event that is entropy between X & Y is greater than Z sometime after evil prankster presses 0 & 1 keys
if both close to 0, means you'll probably die
nonzero probability you see both, but if entangled enough, is extremely close to 0
of course time & computationally expensive, but that's what you get if you try to use SM to answer question this abstract
but there's a deeper problem: even if you had the luxury to do this, partitioning quantum states of the computers is arbitrary (based on what you think a computer is) but partitioning quantum state of "you" is very important b/c that determines what actually happens to you
SM doesn't tell you how to partition quantum state of universe into "you" vs "not you"
is famous unsolved problem called "measurement problem"
doesn't matter much if you want to figure out what atoms in objects are going to do (due to decoherence, everyone usually agrees about what the universe is doing), but matters very much for questions like "0 or 1" and I think it'll become very important once we make sufficiently good brain-computer interfaces
some proposed TOEs such as digital physics are a lot less sensitive to what you think "you" is, so let's discuss that now

hmm, seems like the 2 entropies will probably be similar and correlated in the wrong way, I might need something with phase angles (look up decoherence)
or I can just explain how measurement works then skip to the punchline

might not -> cannot (in title)
```

And if you do the painstaking calculations, they'll tell you that the outcome of this experiment looks something like this:
