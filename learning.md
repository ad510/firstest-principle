...
Is there any sequence that Solomonoff induction *cannot* predict? Yes there is! One example would be a sequence generated by looping over every possible program in your favorite [Turing complete](https://stackoverflow.com/questions/7284/what-is-turing-complete#7320) programming language, and for each program, displaying 0 if it would run forever and displaying 1 [if it would halt](http://scottaaronson.com/democritus/lec3.html). If we can ever calculate that sequence then it would [scientifically demonstrate](http://rationalwiki.org/wiki/Falsifiability) that Solomonoff induction is in fact *not* the most badass learning algorithm we could use, and that it's possible to run an even more badass learning algorithm that is just like Solomonoff induction except that you run the randomly-generated programs using a [programming language that can solve the halting problem](https://esolangs.org/wiki/Banana_Scheme). In fact, we can take this further and define an [infinite hierarchy](http://scottaaronson.com/democritus/lec4.html) of Solomonoff induction-like algorithms where each algorithm is more badass than the ones before it. That said, we [don't know of any way](https://en.wikipedia.org/wiki/Church-Turing_thesis) to actually run any of these more-badass-than-Solomonoff-induction algorithms in the first place.
